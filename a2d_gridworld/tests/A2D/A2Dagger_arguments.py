...
def get_args():
    """
    Get the standard arguments for running the main gridworld experiments.
    ...
    parser.add_argument('--rl-state',               default=int(True), metavar='N', help='01', type=int)   # RL(MDP). Expert.
    parser.add_argument('--a2d-partial-state',      default=int(True), metavar='N', help='06', type=int)   # A2D(Compact) on compact partial state.
    parser.add_argument('--ad-partial-observe',     default=int(True), metavar='N', help='07', type=int)   # AIL on partial, image-based observation.
    parser.add_argument('--a2d-partial-observe',    default=int(True), metavar='N', help='08', type=int)   # A2D(Image) on partial, image-based observation.
    parser.add_argument('--ete-partial-observe',    default=int(True), metavar='N', help='09', type=int)   # PreEnc on partial, image-based observation.
    parser.add_argument('--arl-partial-observe',    default=int(True), metavar='N', help='10', type=int)   # RL(Asym) on partial, image-based observation.
    parser.add_argument('--rl-partial-observe',     default=int(True), metavar='N', help='11', type=int)   # RL on partial, image-based observation.

    # These experiments are for testing and debugging the code.  They aren't the most useful experiments going.
    parser.add_argument('--d-state',                default=int(False), metavar='N', help='02', type=int)   # DAgger.
    parser.add_argument('--a2d-state',              default=int(False), metavar='N', help='03', type=int)   # Symmetric A2D.
    parser.add_argument('--rl-partial-state',       default=int(False), metavar='N', help='04', type=int)   # RL(Compact) on compact partial state.
    parser.add_argument('--ad-partial-state',       default=int(False), metavar='N', help='05', type=int)   # AIL on compact partial state.
    parser.add_argument('--ad-full-observe',        default=int(False), metavar='N', help='12', type=int)   # AD on complete, image-based observation.
    parser.add_argument('--a2d-full-observe',       default=int(False), metavar='N', help='13', type=int)   # A2D on complete, image-based observation.
    parser.add_argument('--rl-full-observe',        default=int(False), metavar='N', help='14', type=int)   # RL on complete, image-based observation.
    parser.add_argument('--arl-full-observe',       default=int(False), metavar='N', help='15', type=int)   # ARL on complete, image-based observation.
    parser.add_argument('--ete-full-observe',       default=int(False), metavar='N', help='16', type=int)   # PreEnc on complete, image-based observation.

    # These are the two environments used in the paper.
    # # NOTE that we hard drop KL to 0.005 for PREENC-TD in the code.
    # parser.add_argument('--env-name', type=str,  default="MiniGrid-TigerDoorEnv-v0", metavar='G', help='')
    # parser.add_argument('--lambda-a2d', type=float, default=0.95, metavar='G', help='GAE parameter lambda to use in A2D.')

    parser.add_argument('--env-name', type=str, default="MiniGrid-LavaGapS7-v0", metavar='G', help='')
    parser.add_argument('--lambda-a2d', type=float, default=0.9, metavar='G', help='GAE parameter lambda to use in A2D.')
    ...
    parser.add_argument('--lambda-rl', type=float, default=0.95, metavar='G', help='GAE parameter lambda to use in straight RL.')
    parser.add_argument('--max-kl', type=float, default=0.01, metavar='G', help='KL step in TRPO.')
    parser.add_argument('--USE-Q', type=int, default=int(False), metavar='G', help='Use a Q fn.  Only works for partial observe.')
    parser.add_argument('--value-calc', type=str, default="gae", metavar='G', help='loss fn for learning value fn, {"time", "gae"}.')
    parser.add_argument('--regularize-adv', type=int, default=int(True), metavar='G', help='directly regularize advantages, or, regularize surrogate loss?')
    parser.add_argument('--rl-entropy-reg', type=float, default=1.0, metavar='G',help='entropy regularization to apply.')
    parser.add_argument('--a2d-entropy-reg', type=float, default=10.0, metavar='G', help='entropy regularization to apply.')
    parser.add_argument('--beta-decay', type=float, default=0.8, metavar='G', help='Rate at which to decay mixture parameter beta.')
    parser.add_argument('--log-interval', type=int, default=5, metavar='N', help='interval between training status logs (default: 10)')
    parser.add_argument('--initialization', type=str, default="orthogonal", metavar='G', help='How to initialize policy networks.')
    ...
    parser.add_argument('--render-type', type=str, default='observe', metavar='G', help='Default render type. ')
    ...
    parser.add_argument('--DO-TRAJECTORY-RENDER', type=int, default=int(False), metavar='G', help='Save pictures? Slows down run.')
    parser.add_argument('--verbose', type=int, default=int(not cluster), metavar='N', help='generate lots of output.')
    ...
    # Parameters for pretrained encoder.
